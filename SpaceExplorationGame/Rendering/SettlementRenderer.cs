...
public static class SettlementRenderer
{
        ...
        PlanetSurfaceData surfaceData, SettlementData settlement)
    {
        ...
        for (int sx = settlement.TileX; sx < settlement.TileX + settlement.Width && sx < surfaceData.Width; sx++)
        {
            for (int sy = settlement.TileY; sy < settlement.TileY + settlement.Height && sy < surfaceData.Height; sy++)
            {
                ...
                renderer.DrawRect(camera, worldPos, GameConfig.TileSize, GameConfig.TileSize, 80, 82, 88);
            }
        }
        ...
        foreach (var (stX, stY, stW, stH) in layout.Streets)
        {
            ...
            renderer.DrawRect(camera, streetCenter, (int)stW, (int)stH, 100, 98, 90);
            ...
            if (stW > stH) // horizontal street
            {
                for (float dx = stX + 8; dx < stX + stW - 8; dx += 16)
                {
                    ...
                    renderer.DrawRect(camera, dashPos, 8, 2, 160, 155, 120, 120);
                }
            }
            else // vertical street
            {
                for (float dy = stY + 8; dy < stY + stH - 8; dy += 16)
                {
                    ...
                    renderer.DrawRect(camera, dashPos, 2, 8, 160, 155, 120, 120);
                }
            }
        }
        ...
        foreach (var (lx, ly) in layout.Lights)
        {
            ...
            renderer.DrawRect(camera, lightPos, 6, 6, 60, 60, 65); // post
            renderer.DrawRect(camera, lightPos + new Vector2(0, -2), 4, 4, 255, 230, 140, 140); // glow
            renderer.DrawRect(camera, lightPos + new Vector2(0, -2), 2, 2, 255, 245, 180); // bulb
        }

        // Perimeter fence (four edges)
        renderer.DrawRect(camera, new Vector2(px + pw / 2f, py), (int)pw, 2, 140, 140, 150);       // top
        renderer.DrawRect(camera, new Vector2(px + pw / 2f, py + ph), (int)pw, 2, 140, 140, 150);   // bottom
        renderer.DrawRect(camera, new Vector2(px, py + ph / 2f), 2, (int)ph, 140, 140, 150);        // left
        renderer.DrawRect(camera, new Vector2(px + pw, py + ph / 2f), 2, (int)ph, 140, 140, 150);   // right

        // Entrance gate marker (gap in top fence with pillars)
        float gateX = px + pw / 2f;
        renderer.DrawRect(camera, new Vector2(gateX, py), 24, 2, 80, 82, 88); // erase fence section
        renderer.DrawRect(camera, new Vector2(gateX - 13, py - 2), 4, 6, 160, 160, 170); // left pillar
        renderer.DrawRect(camera, new Vector2(gateX + 13, py - 2), 4, 6, 160, 160, 170); // right pillar
    ...
    private static void RenderBuilding(SpriteRenderer renderer, Camera camera, SettlementBuilding b)
    {
        ...
        // Building body
        renderer.DrawRect(camera, bCenter, (int)b.W, (int)b.H, b.R, b.G, b.B);

        // Roof edge (darker top strip)
        byte roofR = (byte)Math.Clamp(b.R - 25, 0, 255);
        byte roofG = (byte)Math.Clamp(b.G - 25, 0, 255);
        byte roofB = (byte)Math.Clamp(b.B - 25, 0, 255);
        var roofPos = new Vector2(b.X + b.W / 2f, b.Y + 2);
        renderer.DrawRect(camera, roofPos, (int)b.W, 4, roofR, roofG, roofB);

        // Shadow on bottom/right
        var shadowBottom = new Vector2(b.X + b.W / 2f, b.Y + b.H - 1);
        renderer.DrawRect(camera, shadowBottom, (int)b.W, 3, 40, 40, 45, 100);
        var shadowRight = new Vector2(b.X + b.W - 1, b.Y + b.H / 2f);
        renderer.DrawRect(camera, shadowRight, 3, (int)b.H, 40, 40, 45, 80);
            ...
            for (int wr = 0; wr < b.WindowRows; wr++)
            {
                for (int wc = 0; wc < b.WindowCols; wc++)
                {
                    ...
                    if (ww >= 2 && wh >= 2)
                    {
                        ...
                        renderer.DrawRect(camera, winPos, (int)ww, (int)wh, 180, 200, 140, 180);
                    }
                }
            }
        }
        ...
        if (b.HasAntenna)
        {
            ...
            renderer.DrawRect(camera, antennaBase + new Vector2(0, -6), 2, 12, 170, 170, 180);
            renderer.DrawRect(camera, antennaBase + new Vector2(0, -11), 4, 2, 200, 60, 60); // red tip
        }
        ...
        if (b.HasChimney)
        {
            ...
            renderer.DrawRect(camera, chimneyPos, 6, 6, roofR, roofG, roofB);
        }
    }
}
