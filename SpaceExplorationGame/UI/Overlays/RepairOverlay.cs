...
public class RepairOverlay : OverlayBase
{
    ...
    public override void Render(Game game)
    {
        ...
        // Title
        renderer.DrawTextScreen(panelX + 15, panelY + 10, "REPAIR STATION", new Color3(100, 255, 100), 2.5f);
        ...
        renderer.DrawTextScreen(panelX + 20, panelY + 60, $"SHIP HULL: {game.Player.ShipHealth:F0} / {game.Player.ShipMaxHealth:F0}", new Color3(200, 200, 200), 2f);
        ...
        renderer.DrawRectScreen(barX, barY, barW * (game.Player.ShipHealth / game.Player.ShipMaxHealth), 20, new Color3(100, 255, 100));

        if (damage > 0)
        {
            renderer.DrawTextScreen(panelX + 20, panelY + 130, $"DAMAGE: {damage:F0} POINTS", new Color3(255, 150, 100), 2f);
            renderer.DrawTextScreen(panelX + 20, panelY + 160, $"REPAIR COST: {cost} CREDITS", new Color3(255, 220, 80), 2f);

            bool canAfford = game.Player.Credits >= cost;
            if (canAfford)
                renderer.DrawTextScreen(panelX + 20, panelY + 200, "[ENTER] REPAIR ALL", new Color3(100, 255, 100), 2f);
            else
                renderer.DrawTextScreen(panelX + 20, panelY + 200, "INSUFFICIENT CREDITS", new Color3(255, 80, 80), 2f);
        }
        else
        {
            renderer.DrawTextScreen(panelX + 20, panelY + 130, "HULL INTEGRITY: 100%", new Color3(100, 255, 100), 2.5f);
            renderer.DrawTextScreen(panelX + 20, panelY + 165, "NO REPAIRS NEEDED", new Color3(150, 200, 150), 2f);
        }
