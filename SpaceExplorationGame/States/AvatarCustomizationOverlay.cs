...
/// <summary>
/// Avatar customization overlay: browse equipped avatar slots, swap/buy/sell parts.
/// Two-column layout identical to ShipCustomizationOverlay.
/// </summary>
public class AvatarCustomizationOverlay
{
    ...
    private AvatarPart[] _availableParts = [];
    ...
    private bool IsOwned(PlayerData player, AvatarPart part)
    {
    ...
    private bool IsInInventory(PlayerData player, AvatarPart part) =>
    ...
    private void TryEquipOrBuy(Game game)
    {
        ...
        {
            var inv = player.OwnedAvatarParts.FirstOrDefault(p => p.Id == newPart.Id);
            if (inv != null) player.OwnedAvatarParts.Remove(inv);
            if (current != null) player.OwnedAvatarParts.Add(current);
            player.EquippedAvatarParts[slot] = newPart;
            SetStatus("EQUIPPED!");
        }
        else
        {
    ...
    private void TrySellPart(Game game)
    {
        ...
        {
            bool eq = player.EquippedAvatarParts.Values.Any(p => p.Id == part.Id);
    ...
    {
        _availableParts = AvatarPartCatalog.GetPartsForSlot(SlotOrder[_selectedSlot]);
        _selectedPart = 0;
    }
    ...
    public void Render(Game game, SpriteRenderer renderer)
    {
        ...
        renderer.DrawTextScreen(panelX + 15, panelY + 10, "AVATAR CUSTOMIZATION", 100, 255, 180, 2.5f);
        ...
        for (int i = 0; i < SlotOrder.Length; i++)
        {
            ...
            game.Player.EquippedAvatarParts.TryGetValue(slot, out var equipped);

            float itemH = 55;
            if (selected)
                renderer.DrawRectScreen(panelX + 5, slotY - 3, leftW - 5, itemH, 30, 40, 70, active ? (byte)180 : (byte)100);

            string slotName = AvatarPartCatalog.GetSlotName(slot);
        ...
        renderer.DrawTextScreen(rightX + 5, contentY, $"PARTS: {AvatarPartCatalog.GetSlotName(currentSlot)}", 180, 180, 220, 1.8f);
        game.Player.EquippedAvatarParts.TryGetValue(currentSlot, out var currentEquipped);
        var player = game.Player;
    ...
    private void RenderStatComparison(SpriteRenderer renderer, float x, float y, AvatarPart newPart, AvatarPart oldPart)
    {
